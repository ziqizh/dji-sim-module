...
> 本 fork 默认作为 **短信专用 Dashboard** 使用：页面直接进入短信收发，隐藏蜂窝数据/eSIM 管理入口，并在服务端拒绝切换到 `usbnet=1/2/3`，以避免误开数据漫游。模块仍需在美国成功注册到中国 SIM 所属运营商的漫游合作网络，短信本身不要求开启蜂窝数据。收到的短信会以仅当前用户可读的权限持久化到 `~/Library/Application Support/DJ 4G Hub/sms-messages.json`，并默认保留模块内的短信副本，不做自动删除。
